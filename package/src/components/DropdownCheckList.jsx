...
import * as Utils_ddlSingleLevel from '../Utils_ddlSingleLevel';
import * as Utils_ddlMultiLevel from '../Utils_ddlMultiLevel';
...
export default class DropdownCheckList extends Component {
    ...
    componentWillMount() {
        ...
        let normalizedData = (idName && parentIdName)
            ? Utils_ddlMultiLevel.buildHierarchyCollection(dataSource, flatItems, options)
            : Utils_ddlSingleLevel.normalizeData(dataSource, flatItems, options);
    ...
    onCheckChanged = (e) => {
        ...
        this.props.onCheckItemsChanged(checkedItems);
    }
    ...
    onFilterChange = (value) => {
        let { flatItems } = this.state;
        const { isAbleToCopyPaste, singleSelect } = this.props;

        Utils.filter(value, flatItems, { isAbleToCopyPaste, singleSelect });

        if (isAbleToCopyPaste) {
            this.updateSelectedText();
            this.props.onCheckItemsChanged(Utils.getCheckInfo(singleSelect, flatItems));
        }
    ...
    render() {
        ...
        return (
            <div ref={el => { this.dropdownCheckList = el }}>
                ...
                {listVisible ? <Body 
                    dropdownElement={el => { this.dropdownElement = el }}
                    options={this.props}
